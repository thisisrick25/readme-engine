...
import { createGitHubProvider } from '../src/providers.js';
import type { PluginsConfig } from '../src/types.js';

async function test(): Promise<void> {
  console.log("--- Running local Github test ---");
  ...
  const provider = createGitHubProvider({ octokit, owner: username, repo: process.env.LOCAL_REPO_NAME || '', ref: process.env.LOCAL_REF || '' });
  ...
  await fs.writeFile('tests/local-output-github.md', newContent);

  console.log("--- Test complete ---");
  console.log("Output written to tests/local-output-github.md");
}
